...
api_key = st.sidebar.text_input("OpenAI API Key (Required for Live Generation)", type="password")
...
def build_course_prompt(topic, modules, days):
    ...
    Provide output as JSON with the following structure:
    {{
      "course_title": "{topic}",
      "modules": [
        {{
          ...
          "test": {{
            "multiple_choice": [
              {{"question": "Q1", "options": ["A", "B", "C", "D"], "answer": "A", "explanation": "Why A is correct."}}
            ],
            "open_response_exam": [
...
if st.button("🚀 Generate Complete Course", type="primary"):
    if not course_name:
        ...
        st.warning("Please enter an OpenAI API Key in the sidebar to generate custom content dynamically.")
    else:
        with st.spinner("Building custom curriculum, open-response prompts, and comprehensive assessments..."):
            try:
                ...
                response = client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system", "content": "You are an expert university professor creating rigorous educational content."},
                        {"role": "user", "content": build_course_prompt(course_name, num_modules, days_remaining)}
                    ]
                )
